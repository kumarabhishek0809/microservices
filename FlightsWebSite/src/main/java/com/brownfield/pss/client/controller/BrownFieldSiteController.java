...
import com.brownfield.pss.client.domain.UIData;
...
public class BrownFieldSiteController {
	...
	public String greetingSubmit(@ModelAttribute UIData uiData, Model model) {
		Flight[] flights = restTemplate.postForObject(UrlConstants.SEARCH_SERVICE + "/get", uiData.getSearchQuery(),
				Flight[].class);
	...
	@HystrixCommand(fallbackMethod = "getFlightsFailOver", commandProperties = {
			@HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds", value = "500") })
	public List<Flight> getFlights() throws InterruptedException {
		Thread.sleep(500);
		SearchQuery searchQuery = new SearchQuery("NYC", "SFO", "22-JAN-16");
		return Arrays
				.asList(restTemplate.postForObject(UrlConstants.SEARCH_SERVICE + "/get", searchQuery, Flight[].class));
	}

	public List<Flight> getFlightsFailOver() {
		List<Flight> flights = new ArrayList<Flight>();
		flights.add(new Flight("KL 987", "IND", "PAK", LocalDate.now().toString(), new Fares("100", "AED")));
		flights.add(new Flight("KL 987", "IND", "PAK", LocalDate.now().toString(), new Fares("100", "AED")));
		return flights;
	}
	...
	public String searchBookingForm(Model model) {
		UIData uiData = new UIData();
		// uiData.setBookingid("5"); //Commented by Kumar
		model.addAttribute("uidata", uiData);
	...
	public String searchBookingSubmit(@ModelAttribute UIData uiData, Model model) {
		...
		BookingRecord booking = restTemplate.getForObject(UrlConstants.BOOK_SERVICE + "/get/" + id,
				BookingRecord.class);
		// Added by Kumar
		if (booking == null) { // wrong booking id
			model.addAttribute("message", "Wrong Booking Id " + id);
			return "checkinconfirm";
		} // end of changes made by Kumar
